...
const Projects: React.FC = () => {
  const [isPlayingRob, setIsPlayingRob] = useState(false);
  const [isPlayingBj, setIsPlayingBj] = useState(false);
  const robVideoRef = useRef<HTMLVideoElement>(null);
  const bjVideoRef = useRef<HTMLVideoElement>(null);

  const handleVideoClick = (videoRef: React.RefObject<HTMLVideoElement | null>, setIsPlaying: React.Dispatch<React.SetStateAction<boolean>>) => {
          ...
          <li>
            ...
            <div className="video-container">
              <video 
                ...
                loop 
                muted 
                onClick={() => handleVideoClick(robVideoRef, setIsPlayingRob)}
              >
                <source src="/rob_vid.mp4" type="video/mp4" />
                Your browser does not support the video tag.
              </video>
              {!isPlayingRob && (
                <div className="play-button" onClick={() => handleVideoClick(robVideoRef, setIsPlayingRob)}>
                  ▶
                </div>
              )}
            </div>
          ...
          <li>
            ...
            <div className="video-container">
              <video 
                ...
                loop 
                muted 
                onClick={() => handleVideoClick(bjVideoRef, setIsPlayingBj)}
              >
                <source src="/bj_vid.mp4" type="video/mp4" />
                Your browser does not support the video tag.
              </video>
              {!isPlayingBj && (
                <div className="play-button" onClick={() => handleVideoClick(bjVideoRef, setIsPlayingBj)}>
                  ▶
                </div>
              )}
            </div>
